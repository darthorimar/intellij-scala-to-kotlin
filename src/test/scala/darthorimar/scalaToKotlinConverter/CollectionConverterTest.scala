...
class CollectionConverterTest extends ConverterTestBase {
  ...
  def testSeqMmpty(): Unit =
    doExprTest("""Seq.empty[Int]""".stripMargin, """emptyList<Int>()""".stripMargin)

  def testMkString(): Unit =
    doExprTest("""Seq.empty.mkString("(", ",", ")" )""".stripMargin,
               """emptyList().joinToString(",", "(", ")")""".stripMargin)

  def testSeqTail(): Unit =
    doExprTest("""Seq.empty.tail""".stripMargin, """emptyList().drop(1)""".stripMargin)

  def testSeqInit(): Unit =
    doExprTest("""Seq.empty.init""".stripMargin, """emptyList().dropLast(1)""".stripMargin)

  def testSeqHead(): Unit =
    doExprTest("""Seq.empty.head""".stripMargin, """emptyList().first()""".stripMargin)

  def testSeqApply(): Unit =
    doExprTest("""val s = Seq(1,2)
        |s(0)
      """.stripMargin,
  ...
  def testSeqNotEmpty(): Unit =
    doExprTest("""Seq(1,2).nonEmpty()
      """.stripMargin,
               """listOf(1, 2).isNotEmpty()""".stripMargin)

  def testSeqSize(): Unit =
    doExprTest("""Seq(1,2).size()
      """.stripMargin,
               """listOf(1, 2).size""".stripMargin)

  def testSeqOfOptionFlatten(): Unit =
    doExprTest("""Seq(Some(1),None).flatten""".stripMargin, """listOf(1, null).filterNotNull()""".stripMargin)

  def testStringRepeat(): Unit =
    doExprTest(""" "nya" * 4""".stripMargin, """ "nya".repeat(4)""".stripMargin)

  def testPairConstruct(): Unit =
    doExprTest(""" 1 -> 2 """.stripMargin, """ 1 to 2""".stripMargin)

  def testPairComponents(): Unit =
    doExprTest(""" (1 -> 2)._1 """.stripMargin, """ (1 to 2).first""".stripMargin)
